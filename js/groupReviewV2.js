...
export function initGroupReviewV2() {
  ...
  const fieldDefs = [
    ["collateral_no", "collateral_no", "Collateral#", true],
    ["sheet_label", "sheet_label", "Sheet", true],
    ["field_no", "field_no", "Field No.", true],
    ["change_before_text", "change_before", "변경전", false],
    ["change_after_text", "change_after", "변경후", false]
  ];

  function bodyEl() { return document.getElementById("groupReviewBody"); }
  function badgesEl() { return document.getElementById("groupReviewProjectBadges"); }
  function panelEl() { return document.querySelector('.sheet-panel[data-index="13"]'); }
  ...
  function renderBadges() {
    ...
    `).join("");
    wrap.querySelectorAll("[data-project-id]").forEach(button => {
  ...
  function renderLogin() {
    ...
    body.innerHTML = `
      <div class="grv2">
        <div class="grv2-login">
          <h3>그룹리뷰 로그인</h3>
          <div style="font-size:12px;color:#64748b">그룹리뷰 V2는 로컬 계정으로 로그인합니다.</div>
          <form id="grv2LoginForm">
            <label for="grv2LoginId">아이디</label>
            <input id="grv2LoginId" autocomplete="username" required>
            <label for="grv2Password">비밀번호</label>
            <input id="grv2Password" type="password" autocomplete="current-password" required>
            <button type="submit">로그인</button>
            <div id="grv2LoginError" class="grv2-error"></div>
          </form>
        </div>
      </div>`;
    document.getElementById("grv2LoginForm")?.addEventListener("submit", async event => {
      ...
      try {
        state.user = await api("/auth/login", {
          method: "POST",
          body: JSON.stringify({
            login_id: document.getElementById("grv2LoginId").value,
            password: document.getElementById("grv2Password").value
          })
        });
        await loadProjects();
      } catch (error) {
  ...
  function styleFor(row, styleKey) {
    return row.cell_styles?.[styleKey] || {};
  }
  ...
  function renderMain() {
    ...
    const rowHtml = state.rows.map((row, index) => {
      const cells = fieldDefs.map(([field, styleKey, , centered]) => {
        const style = styleFor(row, styleKey);
        return `<td><div class="grv2-cell ${centered ? "center" : ""} ${canEdit ? "editable" : "readonly"}" ${canEdit ? 'contenteditable="true"' : ""} spellcheck="false" data-row-id="${row.id}" data-field="${field}" data-style-key="${styleKey}" data-original="${escapeHtml(row[field])}" style="${escapeHtml(styleText(style))}">${escapeHtml(row[field])}</div></td>`;
      }).join("");
    ...
    body.innerHTML = `
      <div class="grv2">
        <div class="grv2-userbar">
          <div class="grv2-user"><strong>${escapeHtml(state.user.display_name)}</strong><span class="grv2-role">${escapeHtml(state.user.role)}</span></div>
          <button id="grv2Logout" type="button" class="grv2-btn">그룹리뷰 로그아웃</button>
        </div>
        ${state.project ? `<div class="grv2-project-title">${escapeHtml(projectName)}${state.project.completed ? " · 완료" : ""}</div>` : ""}
        ${!state.project ? '<div class="grv2-note">생성된 리뷰 프로젝트가 없습니다.</div>' : `
          <div class="grv2-tabs">${tabs}</div>
          <div class="grv2-toolbar">
            <select id="grv2Size" ${canEdit ? "" : "disabled"}><option>12</option><option selected>13</option><option>15</option><option>18</option><option>22</option></select>
            <button id="grv2Bold" class="bold" type="button" ${canEdit ? "" : "disabled"}>B</button>
    ...
    body.querySelectorAll(".grv2-cell").forEach(cell => {
      cell.addEventListener("focus", () => selectCell(cell));
      cell.addEventListener("click", () => selectCell(cell));
      cell.addEventListener("blur", () => saveCellText(cell));
    });
  ...
  function selectCell(cell) {
    ...
    status(state.selectedCell ? `${state.sheet?.member_name || ""} · ${state.selectedCell.field}` : "셀을 선택하세요.");
  }
  ...
  async function updateSelectedStyle(key, value) {
    ...
    row.cell_styles[styleKey][key] = value;
  ...
  async function toggleSelectedStyle(key) {
    ...
    row.cell_styles ||= {};
    row.cell_styles[styleKey] ||= {};
    row.cell_styles[styleKey][key] = !row.cell_styles[styleKey][key];
    await persistSelectedStyles();
  }
  ...
  async function loadProjects(preferredProjectId = null) {
    ...
    if (!targetId) {
      state.project = null; state.sheets = []; state.sheet = null; state.rows = [];
      renderMain();
      return;
    }
    ...
    try {
      ...
      for (const sheet of sheets) {
        ...
        const data = rows.map(row => ({
          "Collateral#": row.collateral_no,
          "Sheet": row.sheet_label,
          "Field No.": row.field_no,
          "변경전": row.change_before_text,
          "변경후": row.change_after_text
        }));
        const worksheet = XLSX.utils.json_to_sheet(data.length ? data : [{ "Collateral#": "", "Sheet": "", "Field No.": "", "변경전": "", "변경후": "" }]);
  ...
  return {
    renderGroupReviewUI: refreshSessionAndData,
    refresh: refreshSessionAndData
  };
}
