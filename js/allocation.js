...
export function initAllocation(ctx) {
  ...
  window.editColumnPrompt = function() {
    ...
    if (mode.trim() === "1") {
      ...
      if (project.columns.includes(newTrim) && newTrim !== oldTrim) {
    ...
    if (mode.trim() === "2") {
      if (!confirm(`"${oldTrim}" 항목을 삭제하시겠습니까?`)) return;

      project.columns = project.columns.filter(col => col !== oldTrim);
  ...
  window.deleteAllProjects = async function() {
    try {
      ...
      if (!confirm(`현재 등록된 프로젝트 ${allocationData.projects.length}개를 모두 삭제하시겠습니까?`)) {
        return;
      }

      if (!confirm("정말 전체 프로젝트를 삭제하시겠습니까?\n삭제 후 복구할 수 없습니다.")) {
        return;
      }
      ...
      await setDoc(
        allocationRef,
        removeUndefinedDeep(allocationData),
        { merge: true }
      );
  ...
  function renderAllocationUI() {
        ...
        return `
          <td>
            <input
              ...
              oninput="updateAllocationCell('${project.id}', ${rowIndex}, '${escapeHtml(col)}', this.value)"
            >
          </td>
        `;
      }).join("");
    ...
    body.innerHTML = `
      ...
      <div class="work-info">
        첫 번째 열은 이름 고정이며, 두 번째 열부터 항목 추가로 생성됩니다.
        ☰ 버튼 마우스 클릭&드래그로 순서를 바꿀 수 있습니다.
      </div>

      <div class="work-header-actions">
        <button class="action-btn" onclick="addColumnPrompt()">항목 추가</button>
        <button class="action-btn" onclick="editColumnPrompt()">항목 편집</button>
  ...
  window.dropMemberRow = function(event, projectId, targetIndex) {
    ...
    const movedRow = project.rows.splice(draggedMemberIndex, 1)[0];
